...
class CashMachineGUI:
    ...
    def create_widgets(self):
        title_label = tk.Label(self.root, text="CASH MACHINE", font=("Helvetica", 24, "bold"), bg="#37474F", fg="white")
        ...
        withdraw_button = tk.Button(menu_frame, text="Withdraw Money", command=self.withdraw_money, font=("Helvetica", 18), bg="#2979FF", fg="white")  # Dark blue button
        withdraw_button.grid(row=0, column=0, padx=10)

        deposit_button = tk.Button(menu_frame, text="Deposit Money", command=self.deposit_money, font=("Helvetica", 18), bg="#2979FF", fg="white")  # Dark blue button
        deposit_button.grid(row=0, column=1, padx=10)

        history_button = tk.Button(menu_frame, text="Show History", command=self.show_history, font=("Helvetica", 18), bg="#2979FF", fg="white")  # Dark blue button
        history_button.grid(row=0, column=2, padx=10)

        quit_button = tk.Button(self.root, text="Quit", command=self.quit_program, bg="#d32f2f", fg="white", font=("Helvetica", 18))
        quit_button.pack()
        ...
        # Text widget to display history (increased height to 20) and removed wrap=tk.WORD
        self.history_text = tk.Text(self.history_frame, height=20, width=60, wrap=tk.NONE, font=("Helvetica", 12), bg="#455A64", fg="white")  # Dark text area
    ...
    def withdraw_money(self):
        try:
            ...
            self.update_label(tk.Label(self.root, text=result, font=("Helvetica", 12), bg="#263238", fg="white"), result)
            self.update_label(self.balance_label, self.cash_machine.check_balance())  # Update balance label
        except ValueError:
            self.update_label(tk.Label(self.root, text="Invalid input. Please enter a valid amount.", font=("Helvetica", 12), bg="#263238", fg="white"), "Invalid input. Please enter a valid amount.")
